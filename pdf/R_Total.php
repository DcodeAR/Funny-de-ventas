...
$pdf->Cell(0,60,'Reporte de Carpetas Vencidas',0,0,'C');
$pdf->Ln(5);
$pdf->Cell(196);
...
$pdf->Cell(23,5,'GERENTE',1);
$pdf->Cell(60,5,'EMPRESA',1);
$pdf->Cell(17,5,'RUC',1);
$pdf->Cell(6,5,'PT',1);
...
// $pdf->Cell(35,5,'OBSERVACIONES',1);
$pdf->Cell(23,5,'CONSULTOR',1);
$pdf->Cell(20,5,'SUPERVISOR',1);
$pdf->Ln(5);
$pdf->SetFont('Helvetica','',7);
mysqli_query($conexion,"SET @fila=0");
$usuario=mysqli_query($conexion,"SELECT @fila:=@fila+1 as fila,ca.Dealer,ca.Gerente,ca.Empresa,ca.Ruc,ca.Porta,ca.Operador,ca.Unidades,ca.RentaBasica,
...
while ($row=mysqli_fetch_array($usuario))
{
      ...
      $pdf->Cell(2);
        ...
        $pdf->Cell(8, 5,''.$row['fila'], 1);
        $pdf->Cell(13, 5,''.$row['Dealer'], 1);
        $pdf->Cell(23, 5,''.$row['Gerente'], 1);
        $pdf->Cell(60, 5,''.utf8_decode($row['Empresa']), 1);
        ...
        // $pdf->Cell(35, 5,''.utf8_decode($row['Observacion']), 1);
        $pdf->Cell(23, 5,''.utf8_decode($row['Nombrec'])." ".utf8_decode($row['ApellidoCP']), 1);
        $pdf->Cell(20, 5,''.utf8_decode($row['Usuario'])." ".utf8_decode($row['ApellidoP']), 1);
        $pdf->Ln(5);
