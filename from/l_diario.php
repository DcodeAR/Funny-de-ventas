...
include("menuadmi.php");
	...
	for ($i=1; $i <= $numero; $i++) { 	
	$fechas = "".$i."-".$mes."-2019"; //5 agosto de 2004 por ejemplo  
...
$dd=date('w', $fechats);
	...
	//SUPERVISOR
	$super=mysqli_query($conexion,"SELECT * FROM tusuario WHERE Usuario<>'CESADOS' AND Nivel='editor'");
	while ($row=mysqli_fetch_array($super)) {
	echo "</tr>";	
	echo "<tr>";
			echo "<td style='background:#92cddc; color:#000; font-weight:bold;'>".$row['Usuario']." ".$row['ApellidoP']."</td>";
		for ($i=1; $i <= $numero; $i++) 
		{   $fechasx = "".$i."-08-2019";
            $fechatsx = strtotime($fechasx);
            $ddx=date('w', $fechatsx);
			if ($ddx==0) {
				$query=mysqli_query($conexion,"SELECT v.FechaCierre,SUM(v.Unidades) as total
				FROM tcarpeta v
				INNER JOIN tconsultor c ON c.IdConsultor=v.IdConsultor
				INNER JOIN tusuario s ON s.IdUsuario=c.IdUsuario
				WHERE FechaCierre='19/".$mes."/".$i."' AND v.Estado='5' AND s.IdUsuario='".$row['IdUsuario']."'");
		...
		$query4=mysqli_query($conexion,"SELECT v.FechaCierre,SUM(v.Unidades) as general
			FROM tcarpeta v
			INNER JOIN tconsultor c ON c.IdConsultor=v.IdConsultor
			INNER JOIN tusuario s ON s.IdUsuario=c.IdUsuario
			WHERE v.Estado='5' AND s.IdUsuario='".$row['IdUsuario']."' AND MONTH(v.FechaCierre)='".$mes."'");
			while ($fila7=mysqli_fetch_array($query4)) 
			{
			$result=$fila7['general'];
			if ($result>0)
			 {
	...
	$query=mysqli_query($conexion,"SELECT * 
		FROM tconsultor c
		INNER JOIN tusuario s ON s.IdUsuario=c.IdUsuario
		WHERE s.IdUsuario='".$row['IdUsuario']."' AND s.Usuario<>'CESADOS'
		ORDER BY s.IdUsuario");
	while ($fila=mysqli_fetch_array($query)) {
		echo "<td style='font-size:10px; font-weight:bold;'>".$fila['Nombrec']." ".$fila['ApellidoCP']."</td>";
	

	//carpeta DIARIA POR CONSULTOR

		for ($i=1; $i <= $numero; $i++) { 
		$query3=mysqli_query($conexion,"SELECT v.FechaCierre,SUM(v.Unidades) as total
			FROM tcarpeta v
			INNER JOIN tconsultor c ON c.IdConsultor=v.IdConsultor
			INNER JOIN tusuario s ON s.IdUsuario=c.IdUsuario
			WHERE c.IdConsultor='".$fila['IdConsultor']."' AND v.FechaCierre='19/".$mes."/".$i."' AND v.Estado='5'
			ORDER BY s.IdUsuario");
	...
	//GENERAL POR CONSULTOR
		$query4=mysqli_query($conexion,"SELECT v.FechaCierre,SUM(v.Unidades) as general
			...
			INNER JOIN tconsultor c ON c.IdConsultor=v.IdConsultor
			INNER JOIN tusuario s ON s.IdUsuario=c.IdUsuario
			WHERE c.IdConsultor='".$fila['IdConsultor']."' AND v.Estado='5' AND MONTH(v.FechaCierre)='".$mes."'
			ORDER BY s.IdUsuario");
	...
	//RESULTADO GENERAL POR DIAS
	  echo "<tr>";
		...
		$query=mysqli_query($conexion,"SELECT v.FechaCierre,SUM(v.Unidades) as total
			...
			INNER JOIN tconsultor c ON c.IdConsultor=v.IdConsultor
			INNER JOIN tusuario s ON s.IdUsuario=c.IdUsuario
			WHERE FechaCierre='19/".$mes."/".$i."' AND v.Estado='5'");
			while ($fila6=mysqli_fetch_array($query)) 
			{
				if ($fila6['total']<0) {
		...
		$query4=mysqli_query($conexion,"SELECT v.FechaCierre,SUM(v.Unidades) as general
			...
			INNER JOIN tconsultor c ON c.IdConsultor=v.IdConsultor
			INNER JOIN tusuario s ON s.IdUsuario=c.IdUsuario
			WHERE v.Estado='5' AND MONTH(v.FechaCierre)='".$mes."'");
			while ($fila7=mysqli_fetch_array($query4)) 
			{
			$result=$fila7['general'];
			echo "<td style='background:#403151; color:#fff; text-align:center;'>".$result."</td>";
