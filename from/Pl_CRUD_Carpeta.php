...
if (isset($_POST['guardar'])) 
{
	...
	if ($nmiusario!=0) {
		echo "<script>alert('RUC ingresado por otro Supervisor, favor de enviar correo a sistemas@entel-empresas.pe!');
	...
	else
	{
		...
		$query2="INSERT INTO tiempoconsultor VALUES(TIMEDIFF('".$_COOKIE["final"]."','".$_COOKIE["inicio"]."'),
		'".$_COOKIE["inicio"]."',
		'".$_COOKIE["final"]."',
		'$Estado',
		'".$Consultor."',
...
if (isset($_POST['guardas'])) 
{
	...
	$sql=mysqli_query($conexion,"SELECT * FROM tconsultor c
					inner join tusuario u on u.IdUsuario=c.IdUsuario
					WHERE c.Nombrec='".$_SESSION['username']."'");
	while ($row=mysqli_fetch_array($sql)) 
	{
	$usuario=mysqli_query($conexion,"SELECT * FROM  tcarpeta ca
		inner join tconsultor c on c.IdConsultor=ca.IdConsultor
